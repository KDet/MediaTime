...
{
    public sealed class ClipsRepository : BaseMultimediaRepository, IClipsRepository
    {
        public ClipsRepository(IHtmlPageLoaderService htmlPageLoaderService)
            : base(htmlPageLoaderService)
        {
            Url = string.Format("{0}/video/clips/", BaseUrl);
        }

        public async Task<Media[]> GetMediaAsync(View view, ClipsFilters filters, Sort sort = Sort.Default, int page = 0)
        {
            return view == View.Detailed
                       ? (Media[])(await GetDetailedMediaAsync(filters, sort, page))
                       : await GetListedMediaAsync(filters, sort, page);
        }
        public async Task<MediaDetailed[]> GetDetailedMediaAsync(ClipsFilters filters, Sort sort = Sort.Default, int page = 0)
        {
        ...
        public async Task<MediaListed[]> GetListedMediaAsync(ClipsFilters filters, Sort sort = Sort.Default, int page = 0)
        {
