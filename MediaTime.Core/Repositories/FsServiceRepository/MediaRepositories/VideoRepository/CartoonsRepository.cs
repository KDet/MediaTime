...
{
    public sealed class CartoonsRepository : BaseMultimediaRepository, ICartoonsRepository
    {
        public CartoonsRepository(IHtmlPageLoaderService htmlPageLoaderService)
            : base(htmlPageLoaderService)
        {
            Url = string.Format("{0}/video/cartoons/", BaseUrl);
        }
        ...
        public async Task<Media[]> GetMediaAsync(View view, CartoonsFilters filters, Sort sort = Sort.Default, int page = 0)
        {
            return view == View.Detailed
                       ? (Media[])(await GetDetailedMediaAsync(filters, sort, page))
                       : await GetListedMediaAsync(filters, sort, page);
        }
        public async Task<MediaDetailed[]> GetDetailedMediaAsync(CartoonsFilters filters, Sort sort = Sort.Default, int page = 0)
        {
        ...
        public async Task<MediaListed[]> GetListedMediaAsync(CartoonsFilters filters, Sort sort = Sort.Default, int page = 0)
        {
