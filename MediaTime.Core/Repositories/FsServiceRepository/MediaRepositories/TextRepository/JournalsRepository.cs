...
{
    public sealed class JournalsRepository : LiteratureFilterableRepository, IJournalsRepository
    {
        public JournalsRepository(IHtmlPageLoaderService htmlPageLoaderService) : base(htmlPageLoaderService)
        {
            Url = string.Format("{0}/texts/journals/", BaseUrl);
        }

        public async Task<Media[]> GetMediaAsync(View view, JournalsFilters filters, Sort sort = Sort.Default, int page = 0)
        {
            return view == View.Detailed
                       ? (Media[])(await GetDetailedMediaAsync(filters, sort, page))
                       : await GetListedMediaAsync(filters, sort, page);
        }
        public async Task<MediaDetailed[]> GetDetailedMediaAsync(JournalsFilters filters, Sort sort = Sort.Default, int page = 0)
        {
        ...
        public async Task<MediaListed[]> GetListedMediaAsync(JournalsFilters filters, Sort sort = Sort.Default, int page = 0)
        {
